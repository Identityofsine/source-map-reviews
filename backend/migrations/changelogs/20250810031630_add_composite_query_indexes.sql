...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_map_tags_lk_tag_map_name 
ON map_tags (lk_tag, map_name);

-- Reverse composite index for map-based tag lookups
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_map_tags_map_name_lk_tag 
ON map_tags (map_name, lk_tag);

-- Index for tag popularity analysis
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_map_tags_lk_tag_created 
ON map_tags (lk_tag, created_at DESC);

-- Composite index for map review images (optimizes bulk image loading)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_map_review_images_review_image 
ON map_review_images (map_review_id, image_id);

-- Reverse index for finding reviews by image
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_map_review_images_image_review 
ON map_review_images (image_id, map_review_id);

-- Index for image metadata searches
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_images_path_lower 
ON images (lower(image_path));

-- Index for image caption searches (if users search image descriptions)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_images_caption_trigram 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_maps_created_path 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_map_reviews_distinct_map_names 
ON map_reviews (map_name) WHERE map_review_id IS NOT NULL;

-- Composite index for user activity tracking (reviews + timestamps)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_map_reviews_user_activity 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_map_tags_count_optimization 
ON map_tags (map_name) INCLUDE (lk_tag);

-- Index for tag-based map discovery (finding similar maps)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_map_tags_tag_discovery 
ON map_tags (lk_tag) INCLUDE (map_name, created_at);

-- Partial index for recently tagged maps (trending tags)
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_map_tags_recent 
ON map_tags (lk_tag, created_at DESC) WHERE created_at > (NOW() - INTERVAL '7 days');

-- Index for maps with high review activity
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_maps_review_activity 
ON map_reviews (map_name) INCLUDE (created_at, stars) WHERE created_at > (NOW() - INTERVAL '30 days');
...
DROP INDEX IF EXISTS idx_images_path_lower;
